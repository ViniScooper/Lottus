...
app.post('/products', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    console.log(`[DEBUG] Criando produto: ${name}`, { bgColor_Escolhida: bgColor });
    ...
    console.log(`[DEBUG] Produto criado no DB: ${product.id}`, { bgColor_Salva: product.bgColor });
...
app.put('/products/:id', authMiddleware, async (req, res) => {
  ...
  try {
    let finalCollectionId = collectionId;
    console.log(`[DEBUG] Atualizando produto ${req.params.id}: ${name}`, { bgColor_Enviada: bgColor });
    ...
    console.log(`[DEBUG] Produto atualizado no DB: ${product.id}`, { bgColor_Salva: product.bgColor });
    return res.status(200).json(product);
  } catch (error) {
    return res.status(500).json({ error: error.message });
  }
